...
(defn dataSource
  [{:keys [driver url user password]}]
  (DriverDataSource.
    (contextClassLoader) driver url user password (make-array java.lang.String 0)))

(defn- to-setter [key]
  (str/join (cons "set" (map str/capitalize (str/split (name key) #"-")))))

(defn- invoke-setter [fw key & args]
  (Reflector/invokeInstanceMember (to-setter key) fw (into-array args)))
